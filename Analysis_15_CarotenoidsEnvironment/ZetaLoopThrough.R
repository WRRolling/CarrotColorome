  ...
  if(dim(samps)[1] < 2){
    ...
    for (j in 2:dim(samps)[1]) {  
      ...
      for (i in 1:10) {
        ...
        TmpOut[i,1] <- mean(samp.1$ug_gramfreshZeta, na.rm=T)
        CIVAR <- ci_var(samp.1$ug_gramfreshZeta)
    ...
    Output[2,10] <- mean(samps$ug_gramfreshZeta)
    CIVAR <- ci_var(samps$ug_gramfreshZeta)
...
write.csv(x=Summary.Output.CI, file="ZetaCI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="ZetaMean.csv", row.names = F)
