  ...
  if(dim(samps)[1] < 2){
    ...
    for (j in 2:dim(samps)[1]) {  
      ...
      for (i in 1:10) {
        ...
        TmpOut[i,1] <- mean(samp.1$ug_gramfreshPhyto, na.rm=T)
        CIVAR <- ci_var(samp.1$ug_gramfreshPhyto)
    ...
    Output[2,10] <- mean(samps$ug_gramfreshPhyto)
    CIVAR <- ci_var(samps$ug_gramfreshPhyto)
...
write.csv(x=Summary.Output.CI, file="PhytoCI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="PhytoMean.csv", row.names = F)
