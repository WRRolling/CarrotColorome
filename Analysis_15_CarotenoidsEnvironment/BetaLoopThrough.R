...
for (k in 1:length(All.Unique.Plots)){
    ...
    for (j in 2:dim(samps)[1]) {  
      TmpOut <- matrix(nrow=10,ncol=3)
        for (i in 1:10) {
          ...
          TmpOut[i,1] <- mean(samp.1$ug_gramdryBeta, na.rm=T)
          CIVAR <- ci_var(samp.1$ug_gramdryBeta)
  ...
  Output[2,10] <- mean(samps$ug_gramdryBeta)
  CIVAR <- ci_var(samps$ug_gramdryBeta)
...
write.csv(x=Summary.Output.CI, file="BetaCI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="BetaMean.csv", row.names = F)
