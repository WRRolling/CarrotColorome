  ...
  if(dim(samps)[1] < 2){
    ...
    for (j in 2:dim(samps)[1]) {  
      ...
      for (i in 1:10) {
        ...
        TmpOut[i,1] <- mean(samp.1$avg.ug.fresh450, na.rm=T)
        CIVAR <- ci_var(samp.1$avg.ug.fresh450)
    ...
    Output[2,10] <- mean(samps$avg.ug.fresh450)
    CIVAR <- ci_var(samps$avg.ug.fresh450)
...
write.csv(x=Summary.Output.CI, file="A450CI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="A450Mean.csv", row.names = F)
