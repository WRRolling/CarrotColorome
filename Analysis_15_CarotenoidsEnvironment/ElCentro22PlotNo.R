...
ElCentro22.dat <- HPLC.dat[which(HPLC.dat$Location == "El Centro" & HPLC.dat$Year == 2022),]

# Get average of phenotype of plot 
ElCentro22.Avg.dat <- ElCentro22.dat %>%
  group_by(Plot) %>%
...
# Format Data
for (i in 1:dim(ElCentro22.Avg.dat)[1]) {
  ref <- which(ElCentro22.Avg.dat$Plot[i]== ElCentro22.dat$Plot)
  ElCentro22.Avg.dat[i,7] <- ElCentro22.dat$Genotype[ref[1]]
  Hancock.dat$Genotype[ref[1]]
} 
colnames(ElCentro22.Avg.dat)[7] <- "Genotype"

# Create Object to Store Output
All.Unique.Geno <- unique(ElCentro22.Avg.dat$Genotype)
Summary.Output.Mean <- matrix(nrow=(length(All.Unique.Geno)), ncol=19)
Summary.Output.CI <- matrix(nrow=(length(All.Unique.Geno)), ncol=19)
...
for (k in 1:length(All.Unique.Geno)){
  ...
  samps <- ElCentro22.Avg.dat[which(ElCentro22.Avg.dat$Genotype== All.Unique.Geno[k]),]
  samps <- samps %>%
    drop_na(Beta)
...
write.csv(x=Summary.Output.CI, file="El22.PlotBetaCI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="El22.PlotBetaMean.csv", row.names = F)
...
write.csv(x=Summary.Output.CI, file="El22.PlotAlphaCI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="El22.PlotAlphaMean.csv", row.names = F)
...
write.csv(x=Summary.Output.CI, file="El22.PlotA450CI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="El22.PlotA50Mean.csv", row.names = F)
...
write.csv(x=Summary.Output.CI, file="El22.PlotZetaCI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="El22.PlotZetaMean.csv", row.names = F)
...
write.csv(x=Summary.Output.CI, file="El22.PlotPhytoCI.csv", row.names=F)
write.csv(x=Summary.Output.Mean, file="El22.PlotPhytoMean.csv", row.names = F)
