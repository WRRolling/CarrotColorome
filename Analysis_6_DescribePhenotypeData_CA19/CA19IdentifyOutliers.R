...
####### Part Ten: Location Comparison & Correlation ######
# 10.1 Clean R environment
rm(list = ls())
# 10.2 Load HPLC data and PI anems
WI18 <- read_csv("../Analysis_5_DescribePhenotypeData_WI18/WI18PhenoNoOutLie.csv",
                 col_names=T)
CA19 <- read_csv("FilteredOrangeDat.csv", col_names = T)
which(WI18$Sample.Name %in% CA19$`Sample Name`)
WIPIs <- read_xlsx("../../Text/Supplemental Tables.xlsx", sheet = 2)
CAPIs <- read_xlsx("../../Text/Supplemental Tables.xlsx", sheet = 3)

# 10.3 Subset to only include PIs/Accessions found in both trials
WIPIs <- WIPIs[which(WIPIs$`Sample ID` %in% WI18$Sample.Name),]
WI18[,10] <- WIPIs$`GRIN Accession`
colnames(WI18)[10] <- "PI"
CAPIs <- CAPIs[which(CAPIs$`Plot Number` %in% CA19$`Sample Name`),]
CA19[,13] <- CAPIs$`GRIN Accession`
colnames(CA19)[13] <- "PI"
CA19.1<- CA19[which(CA19$PI %in% WI18$PI),]
CA19.1 <- CA19.1[order(CA19.1$PI),] # same order
WI18.1 <- WI18[which(WI18$PI %in% CA19$PI),]             
WI18.1 <- WI18.1[order(WI18.1$PI),] # same order

# 10.4 Cor test between trials
...
cor.test(WI18.1$Total..ug.g., CA19.1$`Total (ug/g)`,  method = 'spearman')
cor.test(WI18.1$AB.Ratio, CA19.1$AB.Ratio,  method = 'spearman')

# 10.5 Means and Standard Deviations
mean(WI18.1$Alpha..ug.g.)
sd(WI18.1$Alpha..ug.g.)
mean(WI18.1$Beta..ug.g.)
sd(WI18.1$Beta..ug.g.)
...
mean(WI18.1$Total..ug.g., na.rm=T)
sd(WI18.1$Total..ug.g., na.rm=T)
